...
/**
 * This class represents a simple function to convert a folder with CSV files
 * into a GIS_Project and then into a KML file
 * 
...
public class MultiCSV {
	...
	/**
	 * 
	 * This function scans a folder recursively and converts all the CSV files in it
	 * into a toGIS_project by using a function called createProject
	 * 
	...
	public static void ProjectToKML(GIS_project myProject, String outputAdress) {
		...
		String kmlFolderName = "<name>";
		String kmlFolderNameClose = "</name>\n";
	...
	/**
	 * This is a helper function to create a toGIS_project from an ArrayList of
	 * string holding the path for
	 * 
	 * @param myFiles
	 * @return
	 */
	...
	public static toGIS_project createProject(ArrayList<String> myFiles) {
		...
		for (int i = 0; i < myFiles.size(); i++) {
			ArrayList<String[]> tmp = CSVreader.reader(myFiles.get(i), 1);
			Set<GIS_element> mySet = new HashSet<>();
			GIS_layer myLayer = new toGIS_layer(mySet);
			for (int j = 0; j < tmp.size(); j++) {
				toGIS_element tmpElement = new toGIS_element(tmp.get(j), 6, 7, 8);
