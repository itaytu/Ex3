 ...
 * 
 * Some of the functions included are: add, distance3D, vector3D,
 * isValid_GPS_point etc.
 * 
...
public class MyCoords implements coords_converter {
	...
	public double distance3d(Point3D gps0, Point3D gps1) {
		...
		double distance = Math.sqrt((Dis_x * Dis_x) + (Dis_y * Dis_y) + (Dis_z * Dis_z));
		return distance;
	}
	...
	public boolean isValid_GPS_Point(Point3D p) {
		return ((-180 <= p.x() && p.x() <= 180) && (-90 <= p.y() && p.y() <= 90) && (-450 <= p.z() && p.z() <= 9000));
	}
